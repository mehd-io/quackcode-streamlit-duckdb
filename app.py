...
con = duckdb.connect(database=f"data/{DATABASE_NAME}.db", read_only=True)
# motherduck
# con = duckdb.connect(database='md:{DATABASE_NAME}', read_only=True)
...
min_date, max_date = con.execute(
    f"SELECT MIN(timestamp_day), MAX(timestamp_day) FROM {DATABASE_NAME}.main.daily_stats"
).fetchone()
...
SELECT 
    ...
    python_version,
    cpu
FROM {DATABASE_NAME}.main.daily_stats
...
df_python_version = (
    df.groupby(["month", "python_version"])["total_downloads"].sum().unstack().fillna(0)
)
